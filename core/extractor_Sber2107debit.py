...
class Sber2107Debit(Extractor):
    ...
    def get_period_balance(self)->str:
        ...
        res = re.search(
            r'ОСТАТОК НА.*?ОСТАТОК НА.*?ВСЕГО СПИСАНИЙ.*?ВСЕГО ПОПОЛНЕНИЙ.*?\n(.*?)\n',
...
if __name__ == '__main__':
    ...
    for entry in sber2107debit_converter.get_entries():
        print('*'*20)
        pprint(entry)

    print(f"check_support = {sber2107debit_converter.check_support()}")