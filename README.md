...
Расширяемая утилита для конвертация различных вариантов выписок Cбербанка из формата PDF в формат Excel. 
...
![Sberbank2Excel.PNG](docs/Sberbank2Excel.PNG)
...
- [Установка](#установка)
  - [Опция 1: Установка готового файла (пока что доступна только для Windows)](#опция-1-установка-готового-файла-пока-что-доступна-только-для-windows)
  - [Опция 2: Более сложная установка (должна работать на всех платформах)](#опция-2-более-сложная-установка-должна-работать-на-всех-платформах)
- [Конвертация выписки банка.](#конвертация-выписки-банка)
- [ЧаВо](#чаво)
...
- [Приложения](#приложения)
  ...
  - [Приложение В. Использование утилиты в режиме командной строки](#приложение-в-использование-утилиты-в-режиме-командной-строки)
  - [Приложение Г. Установка используя инструменты python из исходного кода.](#приложение-г-установка-используя-инструменты-python-из-исходного-кода)
    - [Вариант "Изначальная установка"](#вариант-изначальная-установка)
    - [Вариант "Апгрейд существующей установки до новой версии"](#вариант-апгрейд-существующей-установки-до-новой-версии)


## Функционал

-   Конвертация одного или многих файлов PDF - выписки за один раз
-   Работает на Windows, а начиная с релиза 5.0.0 также доступна относительно простая установка на macOS и Linux
...
См. [Приложение Г](#приложение-г-установка-используя-инструменты-python-из-исходного-кода)


## Конвертация выписки банка.

**Шаг 1** Запустите `sberbank2Excel.exe` двойным щелчком мыши.  
Прим.: если был использовать способ установки, описанный в [Приложении Г](#приложение-г-установка-используя-инструменты-python-из-исходного-кода)
то необходимо из командной строки (из любой директории) ввести команду   ```sberbankExcel```

*Результат:* приложение запустится в режиме Графического Интерфейса Пользователя (ГИП)
...
См. также [Приложение В. Запуск приложения из командной строки](#приложение-в-использование-утилиты-в-режиме-командной-строки)
...
-   При использовании вариант установки, описанного в [Приложении Г](#приложение-г-установка-используя-инструменты-python-из-исходного-кода) в дополнение к этому можно запускать утилиту на macOS и Linux.
...
-   Анонимизированный скриншот выписки для целей дальнейшей документации. См. примеры [здесь](docs/format_examples)
...
-----------------------------------------------------------------------------------------------------------
| Банк     | Формат[^1]  | Описание | MasterCard | VISA | Трансакции в иностранной валюте | Известные проблемы |
|----------|---------|----------|------------|------|---------------------------------|--------------------|
| Сбербанк | [SBER_DEBIT_2005](/docs/format_examples/SBER_DEBIT_2005.png) | Дебетовая карта образца мая 2020 года  | OK  | ???  |ОК  |   |
| Сбербанк | [SBER_DEBIT_2107](/docs/format_examples/SBER_DEBIT_2107.png) | Дебетовая карта образца июля 2021 года  | OK | ???  | ОК |   |
| Сбербанк | [SBER_CREDIT_2110](/docs/format_examples/SBER_CREDIT_2110.png) | Кредитная карта образца октября 2021 года (ТДВФН) | ???  | OK   | OK? | [13](https://github.com/Ev2geny/Sberbank2Excel/issues/13) |
| Сбербанк | [SBER_PAYMENT_2208](/docs/format_examples/SBER_PAYMENT_2208.png)| Платёжный счёт образца августа 2022 года (ТДВФН)  | НП         | НП   | НП? |
| Сбербанк | [SBER_DEBIT_2212](/docs/format_examples/SBER_DEBIT_2212.png) | Дебетовая карта образца декабря 2022 года | OK         | ???  | ОК  |
| Сбербанк | [SBER_PAYMENT_2212](/docs/format_examples/SBER_PAYMENT_2212.png) | Платёжный счёт образца декабря 2022 года | НП | НП | ОК |
| Сбербанк | [SBER_SAVING_2303](/docs/format_examples/SBER_SAVING_2303.png) | Выписка из лицевого счёта по вкладу «Сберегательный счет» образца марта 2023 года (ТДВФН). Также должно работать с другими типами сберегательных счетов (Активное долголетие и т.д.) | НП  | НП   | НП |                                                                |
| Сбербанк | [SBER_DEBIT_2303_CHELYABINSK](/docs/format_examples/SBER_DEBIT_2303_CHELYABINSK.png) | История операций по дебетовой карте за период. Образца марта 2023 года (ТДВФН). ???Челябинск??  | ОК         | ?    | НЕТ||
| Сбербанк | [SBER_PAYMENT_2406](/docs/format_examples/SBER_PAYMENT_2406.png)| Платёжный счёт образца июня 2024 года   | НП         | НП   | ОК| [43](https://github.com/Ev2geny/Sberbank2Excel/issues/43) |
| Сбербанк | [SBER_PAYMENT_2407](/docs/format_examples/SBER_PAYMENT_2407.png)| Платёжный счёт образца июля 2024 года  | НП | НП   | ОК| [52](https://github.com/Ev2geny/Sberbank2Excel/issues/52)|
...
| Сбербанк | [SBER_PAYMENT_DEBIT_2604b_pay](/docs/format_examples/SBER_PAYMENT_DEBIT_2604b_pay.png) <br> [SBER_PAYMENT_DEBIT_2604b_debit](/docs/format_examples/SBER_PAYMENT_DEBIT_2604b_debit.jpg)| Платёжный счёт а так же Выписка по счёту дебетовой карты образца апреля 2026 года, вариант Б | НП | НП   | ОК| |
| Сбербанк | [SBER_CREDIT_2605](/docs/format_examples/SBER_CREDIT_2605.png)| Кредитная карта образца мая 2026 года  | НП|НП  | ОК? |  |
...
-   При ошибке конвертации конвертер создаст промежуточный текстовый файл с
    ...
    [Инструкция](docs/Anonymisation%20instructions.png) показывает что можно
    удалять, что можно заменять, а что нужно оставить без изменений.
-    Используйте текстовый редактор и
    [инструкцию](docs/Anonymisation%20instructions.png) чтобы удалить
...
-   *Старайтесь удалять или менять как можно меньше информации*. На выходе
    должно получиться что-то типа этого: [пример анонимизированного промежуточного текстового файла](docs/_SBER_DEBIT_2107_anonymized_reduced.txt)
-   Попытайтесь сконвертировать теперь уже анонимизированный текстовый файл
    используя всё тот же sberbank2Excel (для этого на **Шаге 2** при
...
Помимо работы в рижиме Графического Интерфейса Пользователя (ГИП) утилита поддерживает режим работы из командной строки.  
Для этого надо запускать утилиту из командной строки и указать и задать один или более аргументов. Например: 
...
[Приложение Г. Установка используя инструменты python из исходного года.](#приложение-г-установка-используя-инструменты-python-из-исходного-года)
то команду `sberbank2Excel` можно запускать из любой директории.

Подробную инструкцию использования приложения из командной строки можно получить запустив её с аргументом -h 
...
Данная установка слегка более сложная, но должна работать для любой среды (Windows, macOS, Linux)


#### Вариант "Изначальная установка"

1.  Установите [uv](https://docs.astral.sh/uv/getting-started/installation/#installation-methods) из командной строки.
...
1.  Установите Sberbank2Excel из исходного года, использую uv:   
    ```
    uv tool install git+https://github.com/Ev2geny/Sberbank2Excel.git
    ```
1.  Убедитесь, что установка прошла успешно, для этого запустите приложение из командной строки в режиме командной строки
   
    ```
    sberbank2Excel -h
    ```
1.  Попробуйте запустить пиложение из командной строки в графическом режиме
    
    ```
    sberbank2Excel
    ```
    Прим.: в некоторых случаях пользователи сообщают о [проблемах запуска программы в режиме ГИП](https://github.com/Ev2geny/Sberbank2Excel/issues/75). 
    В этом случае придётся использовать программу в режиме командной строки, как описано в [Приложении В.](#приложение-в-использование-утилиты-в-режиме-командной-строки)


#### Вариант "Апгрейд существующей установки до новой версии"

Уже установленную при помощи uv программу можно обновить до последней версии используя команду
...
---
[^1]: На картинке формата выписки как правило красным цветом выделены элементы, отличающий этот формат от предыдущего формата такого же варианта выписки. Например для [SBER_PAYMENT_2604](https://github.com/Ev2geny/Sberbank2Excel/blob/master/docs/format_examples/SBER_PAYMENT_2604.png) показано чем он отличается от [SBER_PAYMENT_2510](https://github.com/Ev2geny/Sberbank2Excel/blob/master/docs/format_examples/SBER_PAYMENT_2510.png).
